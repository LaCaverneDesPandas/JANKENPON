...
def jouer(ton_coup):
    ...
    c.send("A toi".encode(format))
    message = c.recv(1024).decode(format)
    mon_coup = moncoup(message)
    if mon_coup==1:
...
def end():
	fenetre.destroy()
	c.send("quit".encode(format))

bouton5 = Button(fenetre,text='Quitter',command=end)
